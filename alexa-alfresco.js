...
function getWelcomeResponse(callback) {
    ...
    var sessionAttributes = {"name":"greg"};
...
function enumerateTasks(tasksObj) {
    ...
    var response = "";
	for (i = 0; i < count; i++) { 
		var taskName = tasksObj.list.entries[i].entry.name + " for " + tasksObj.list.entries[i].entry.description ;
		var dueDate = simplifyDate(tasksObj.list.entries[i].entry.dueAt);
		if (i < count-1) {
	       response += "Task number: " + tasksObj.list.entries[i].entry.id + ".  " + taskName + " due on " + dueDate + ".  "; 
	    } else {
	       response += " and " + "Task number: " + tasksObj.list.entries[i].entry.id + ".  " + taskName + "due on " + dueDate + ".  "; 
	    }
